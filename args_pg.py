...
def parse_args():
    ...
    parser.add_argument("--exp_rounds", type=int, default=50)
    # param baseline
    param_rng = []
    ...
    baseparam.update(dict(n_estimators=300))
    # params
    # LR
    ...
    max_de_rng = {"name": "max_depth","type": "range","bounds": [2,7],"value_type": "int",'log_scale':False}
    param_rng.append(max_de_rng)

    # min_child_weight
    #baseparam.update(dict(min_child_weight=2))
    min_c_we_rng = {"name":'min_child_weight',"type": "range","bounds": [2,7],"value_type": "int",'log_scale':False}
    param_rng.append(min_c_we_rng)

    # gamma
    #baseparam.update(dict(gamma=0.05))
    #gamma_rng = {'name':'gamma',"type": "range","bounds": [5e-1,5],"value_type": "float",'log_scale':True}
